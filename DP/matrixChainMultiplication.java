...
public class matrixChainMultiplication {
    public static int mcmRe(int arr[], int i, int j) {
        if(i == j) {
        ...
        for(int k=i; k<=j-1; k++) {
            int cost1 = mcmRe(arr, i, k); // Ai...Ak => arr[i-1] x arr[k]
            int cost2 = mcmRe(arr, k+1, j); // Ak+1...Aj => arr[k] X arr[j]
    ...
    public static void main(String[] args) {
        int arr[] = {1, 2, 3, 4, 3};
        // int n = arr.length;
        // System.out.println(mcmRe(arr, 1, n-1));
